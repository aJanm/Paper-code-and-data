...
void USARTx_Config(void)
{
	...
	USART_InitTypeDef USART_InitStructure;//22
	
	/* config USART1 clock */
	macUSART_APBxClock_FUN(macUSART_CLK, ENABLE);//22 对应RCC_APB2PeriphClockCmd
	macUSART_GPIO_APBxClock_FUN(macUSART_GPIO_CLK, ENABLE);//22
	//2022-05-16 复位串口1
		
	/* USART1 GPIO config */
	/* Configure USART1 Tx (PA.09) as alternate function push-pull */
	GPIO_InitStructure.GPIO_Pin =  macUSART_TX_PIN; // GPIO_Pin_9; 
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP; // //复用推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(macUSART_TX_PORT, &GPIO_InitStructure);    //初始化PA9  
	/* Configure USART1 Rx (PA.10) as input floating */
	GPIO_InitStructure.GPIO_Pin = macUSART_RX_PIN;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING;//浮空输入
	GPIO_Init(macUSART_RX_PORT, &GPIO_InitStructure);	//初始化PA10
